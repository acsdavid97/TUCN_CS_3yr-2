...
/*
 * Read an integer from the stdin, and add that value to all pixels in a grayscale picture.
 */
void additiveFactor()
{
	int additiveValue;
	printf("please enter an additive value:");
	if (scanf("%d", &additiveValue) != 1)
	{
		printf("not a number\n");
		return;
	}
	...
	while (openFileDlg(fname))
	{
		...
		for(int i = 0; i < src.rows; i++)
		{
			...
			{
				const uchar pixelValue = src(i, j);
				const int addedValue = pixelValue + additiveValue;
				if (addedValue > 255)
				{
					dst(i, j) = 255;
...
void multiplicativeFactor()
{
	...
	printf("please enter a multiplicative value:");
	if (scanf("%lf", &multiplyFactor) != 1)
	{
		printf("not a number\n");
		return;
	}
	...
	while (openFileDlg(fname))
	{
		...
		for(int i = 0; i < src.rows; i++)
		{
			...
			{
				const uchar pixelValue = src(i, j);
				const double multipliedValue = pixelValue * multiplyFactor;
				if (multipliedValue > 255)
				{
					dst(i, j) = 255;
		...
		imshow("original image", src);
		imshow("changed image", dst);
		imwrite("multiplicative.bmp", dst);
		waitKey();
	}
}
...
void colorImage()
{
	...
	const Vec3b red = Vec3b(0x00, 0x00, 0xff);
	const Vec3b green = Vec3b(0x00, 0xff, 0x00);
	const Vec3b yellow = Vec3b(0x00, 0xff, 0xff);

	Mat_<Vec3b> dst = Mat_<Vec3b>(rows, cols);
	...
	imshow("changed image", dst);
	imwrite("colored_image.bmp", dst);
	waitKey();
}

void inverseMat()
{
	Mat_<float> floatMat = Mat_<float>(3, 3);
	floatMat(0, 0) = 2;
	floatMat(0, 1) = -2;
	floatMat(0, 2) = 5;
	floatMat(1, 0) = 9;
